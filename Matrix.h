...
public:
    ...
    // воод матрицы из файла DONE
    void inputMatrix(const string& path){
        ifstream file(path);
        if (!file.is_open()){
    ...
    // вывод матрицы в файл DONE
    void outputMatrix(const string& path){
        fstream file(path);
        if (!file.is_open()){
    ...
    // элементарное преобразование 1 (смена строк местами) DONE
    ...
    // элеметраное преобразование 2 (умножение на значение отличное от 0) DONE
    ...
    // элеметраное преобразование 3 (сложение строки на строку, умноженную на значение отличное от 0) DONE
    ...
    // * умножение матрицы на скаляр DONE
    ...
    // == проверка равенства матриц - DONE
    ...
    // == проверка равенства матрицы и скаляра DONE
    ...
    // !- проверка неравенства матрицы и скаляра DONE
    bool operator!=(const double c){
        return !(*this==c);
    }

    // != проверка неравенства матриц - DONE
    bool operator!=(const Matrix& A){
        return !(*this==A);
    }
    ...
    // вычисление минора
    double minor(int row, int col){ // вычеркнутые строка и колонка
...
//        A.outputMatrix();
    ...
    [[maybe_unused]]Matrix transposed(){
        if (!isCube()){
            cerr << "Matrix must be square\n";
            exit(1);
        }
